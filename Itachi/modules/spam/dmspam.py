...
from Itachi.modules.help import *
import os
import sys
import asyncio
...
async def dmraid(xspam: Client, e: Message):
      """ Module: Dm Raid """
      Itachi = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(Itachi) == 2:
          ok = await xspam.get_users(Itachi[1])
          id = ok.id
          if int(id) in VERIFIED_USERS:
                text = f"Chal Chal baap Ko mat sikha😈"
                await e.reply_text(text)
          elif int(id) in SUDO_USERS:
                text = f"Abe Lawde that guy part of ◉ 𝐖𝐀𝐑𝐙𝐎𝐍𝐄 ◉."
                await e.reply_text(text)
          else:
              counts = int(Itachi[0])
